...
sub generate_urls {
    ...
    my $url_click_redirect = sprintf(
        "https://%s/dynclick/%s/?social-publisher=%s&social-name=%s-%s&social-location=%s-%s&social-creative=%s-%s&social-creativetype=%s&eseg-name=%s&eseg-item=%s&social-mediaplan=%s&ea-android-adid=%s&ea-ios-idfa=%s&eurl=%s",
    ...
    my $url_impression = sprintf(
        '<img src="https://%s/dynview/%s/1x1.b?social-publisher=%s&social-name=%s-%s&social-location=%s-%s&social-creative=%s-%s&social-creativetype=%s&eseg-name=%s&eseg-item=%s&social-mediaplan=%s&ea-android-adid=%s&ea-ios-idfa=%s&ea-rnd=[RANDOM]" border="0" width="1" height="1" />',
        $params->{dominio_tracking}, $params->{sitio}, $params->{nombre_soporte}, $params->{nombre_soporte}, $params->{nombre_campania},
        $params->{nombre_ubicacion}, $params->{formato_banner}, $params->{nombre_banner}, $params->{formato_banner}, $params->{formato_banner},
        $params->{nombre_segmento} // '', $params->{valor_segmento} // '', $params->{nombre_plan_medios} // '',
    ...
    my $url_click_params = sprintf(
        "%s?social-publisher=%s&social-name=%s-%s&social-location=%s-%s&social-creative=%s-%s&social-creativetype=%s&eseg-name=%s&eseg-item=%s&social-mediaplan=%s&ea-android-adid=%s&ea-ios-idfa=%s",
...
sub new {
    ...
    $self->{canal} = 'Social'; # Nombre del canal
    return $self;
}
