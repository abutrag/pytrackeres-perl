package ColaboracionURLGenerator;
...
sub generate_urls {
    ...
    my $url_click_redirect = sprintf(
        "https://%s/dynclick/%s/?colab-publisher=%s&colab-name=%s-%s&colab-location=%s-%s&colab-creative=%s-%s&colab-creativetype=%s&eseg-name=%s&eseg-item=%s&colab-mediaplan=%s&ea-android-adid=%s&ea-ios-idfa=%s&eurl=%s",
    ...
    my $url_impression = sprintf(
        '<img src="https://%s/dynview/%s/1x1.b?colab-publisher=%s&colab-name=%s-%s&colab-location=%s-%s&colab-creative=%s-%s&colab-creativetype=%s&eseg-name=%s&eseg-item=%s&colab-mediaplan=%s&ea-android-adid=%s&ea-ios-idfa=%s&ea-rnd=[RANDOM]" border="0" width="1" height="1" />',
        $params->{dominio_tracking}, $params->{sitio}, $params->{nombre_soporte}, $params->{nombre_soporte}, $params->{nombre_campania},
        $params->{nombre_ubicacion}, $params->{formato_banner}, $params->{nombre_banner}, $params->{formato_banner}, $params->{formato_banner},
        $params->{nombre_segmento} // '', $params->{valor_segmento} // '', $params->{nombre_plan_medios} // '',
    ...
    my $url_click_params = sprintf(
        "%s?colab-publisher=%s&colab-name=%s-%s&colab-location=%s-%s&colab-creative=%s-%s&colab-creativetype=%s&eseg-name=%s&eseg-item=%s&colab-mediaplan=%s&ea-android-adid=%s&ea-ios-idfa=%s",
...
sub new {
    ...
    $self->{canal} = 'Colaboracion'; # Nombre del canal
    return $self;
}
