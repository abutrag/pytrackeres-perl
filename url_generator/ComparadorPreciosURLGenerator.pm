package ComparadorPreciosURLGenerator;
...
sub validate_params {
    ...
    my @required_params = qw(dominio_tracking sitio nombre_soporte nombre_campania nombre_banner url_destino);
...
sub generate_urls {
    ...
    my $url_click_redirect = sprintf(
        "https://%s/dynclick/%s/?comp-publisher=%s&comp-name=%s-%s&comp-creative=%s&eseg-name=%s&eseg-item=%s&comp-mediaplan=%s&ea-android-adid=%s&ea-ios-idfa=%s&eurl=%s",
        $params->{dominio_tracking}, $params->{sitio}, $params->{nombre_soporte}, $params->{nombre_soporte}, $params->{nombre_campania},
        $params->{nombre_banner}, $params->{nombre_segmento} // '', $params->{valor_segmento} // '', $params->{nombre_plan_medios} // '',
...
sub new {
    ...
    $self->{canal} = 'Comparador'; # Nombre del canal
    return $self;
}
